...
def get_etl(symbol: str, window: int = Query(DEFAULT_WINDOW, ge=MIN_WINDOW, le=MAX_WINDOW)): #window is bounded between 1 and 60
    ...
    return {
        "message": f"ETL complete for {symbol.upper()}",
        "data_points": len(df),
        "saved_to": file_path
    }
...
def get_moving_average(symbol: str, window: int = Query(DEFAULT_WINDOW, ge=DEFAULT_WINDOW, le=MAX_WINDOW)): 
    ...
    return {
        "message": f"moving average complete for {symbol.upper()}",
        "data_points": len(df),
        "saved_to": file_path
    }
...
# Formula: Sharpe Ratio = E(Rp - Rf) / Standard Deviation
...
def get_sharpe_ratio(symbol: str, period: str):
    df = extract.get_historical_data(symbol)
    df = transform.add_volatility(symbol, period, True)
    df["Return"] = df["Close"].pct_change() # Rp: Return of portfolio or asset
    annual_rf = 0.05 # Rf: Risk free rate, can assume here at 5%
    df["Rf"] = (1 + annual_rf) ** (1/252) - 1
    df["Excess Return"] = df["Return"] - df["Rf"] # return - risk free rate
    annualized_excess_return = df["Excess Return"].mean() * 252
    sharpe_ratio_annual = annualized_excess_return / df["Volatility"]

    return {
        "symbol": symbol.upper(),
        "message": f"annualized sharpe ratio for {symbol.upper()} is {sharpe_ratio_annual}"
    }
